...
fun <T : Any> Activity.bindBundle(key: String, default: T? = null): ReadOnlyProperty<Activity, T> {
	return if (default == null) {
		...
		requiredDefault(key, default, finder)
	}
}

fun <T : Any> Fragment.bindArgument(
	key: String,
	default: T? = null
...
fun <T> Activity.bundleOrNull(key: String): ReadOnlyProperty<Activity, T?> =
	getOrNull(key, finder)
...
private fun <T, W : Any?> required(key: String, finder: T.(String) -> Any?) =
	Lazy { type: T, description: KProperty<*> ->
		type.finder(key) as W ?: bundleNotFound(
			key,
			description
		)
	}
...
private fun <T, W : Any?> requiredDefault(key: String, default: W, finder: T.(String) -> Any?) =
	Lazy { type: T, _: KProperty<*> ->
		type.finder(key) as W ?: default
	}

@Suppress("UNCHECKED_CAST")
private fun <T, W : Any?> getOrNull(key: String, finder: T.(String) -> Any?) =
	Lazy { type: T, _: KProperty<*> ->
